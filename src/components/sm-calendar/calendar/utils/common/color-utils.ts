...
function getDarkenColor(r: number, g: number, b: number) {
  // pad each with zeros and return
  return '#' + padZero((r - 46).toString(16)) + padZero((g - 24).toString(16)) + padZero((b - 17).toString(16));
}
...
export function getBorderColor (bgColor: string) {
  if (colourNameToHex(bgColor))
...
export function getBgColor (textColor: string) {
  if (colourNameToHex(textColor))
    return invertColor(colourNameToHex(textColor));
  else if (isValidHexColor(textColor))
    return invertColor(textColor);
  else if(isValidRgbColor(textColor)) {
    let rgb = getRGBArray(textColor);
    return getInvertedColor(rgb[0], rgb[1], rgb[2]);
  }
}

function getRGBArray(bgColor: string) {
  let rgb = bgColor.replace(/[^\d,]/g, '').split(',');
  return rgb.map((c) => parseInt(c));
}

function isValidHexColor(bgColor: string) {
  let regex = /^#([0-9A-F]{3}){1,2}$/i
  return bgColor.indexOf('#') !== -1 && regex.test(bgColor)
}

function isValidRgbColor (bgColor: string) {
